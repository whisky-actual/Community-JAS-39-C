...
symbols_strings_new = {
	...
	["p-19 s-125 sr"] = "FF",       -- P-19 		   - "Flat Face" radar (FF),
	["snr s-125 tr"] = "SA3",        -- SNR S-125 Neva - "Low Blow" radar (LB).		
	["SNR_75V"] = "SA2",             -- SNR S-75 Волхов Fan Song
...
--- Returns the threat string for a given unit type.
--- @param unitType string The unit type to get the threat string for.
--- @return string threatSTR The threat string for the given unit type.
function getThreatSTR(unitType)
	local threatSTR = symbols_strings_new[unitType]
	
	if threatSTR ~= nil then
		if string.match(threatSTR, "^%d+$") == nil then -- Returns nil if threatSTR is not only digits
			return tostring(threatSTR)
		else
			return " " .. tostring(threatSTR) .. " "
		end
	else
