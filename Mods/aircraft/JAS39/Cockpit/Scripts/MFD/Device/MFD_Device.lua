...
local SPEEDOMETER_IAS = get_param_handle("SPEEDOMETER_IAS")
...
function update()
	...
	local curias = sensor_data.getIndicatedAirSpeed() * 1.94384449         -- m/s to kts
	...
	if curias <= 100 then
		SPEEDOMETER_IAS:set(.35 * curias)
	elseif curias > 100 and curias < 150 then
		SPEEDOMETER_IAS:set(.9 * curias - 55)
	elseif curias >= 150 then
		SPEEDOMETER_IAS:set(.4 * curias + 20)
	end
	...
	TGT_NEEDLE:set(math.min(.0000735294 * TGT_STATE^2 + .214706 * TGT_STATE, 200))


	RPM_NEEDLE:set(math.min((1 / 60) * sensor_data.getEngineLeftRPM()^2 + .5 * sensor_data.getEngineLeftRPM(), 209))
	-- get_param_handle("TGTDial"):set(math.min(.0000735294 * TGT_STATE^2 + .214706 * TGT_STATE, 200))
