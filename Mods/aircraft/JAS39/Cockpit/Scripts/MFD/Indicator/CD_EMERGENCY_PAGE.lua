...
CD_EMGY_PAGE 			= CreateElement "ceSimple"
CD_EMGY_PAGE.init_pos	= {0,0}
CD_EMGY_PAGE.name		= create_guid_string()
CD_EMGY_PAGE.element_params = {"CD_EMGY_MODE", "mainpower"}
CD_EMGY_PAGE.controllers    = {{"parameter_compare_with_number",0, 1}, {"parameter_compare_with_number", 1, 1}}
AddElement(CD_EMGY_PAGE)		
...
ADI_EMGY_BACKGROUND.element_params  = {"ADI_ATTITUDE"}
ADI_EMGY_BACKGROUND.controllers		= {{"move_up_down_using_parameter",0, 0.036}}
...
ADI_EMGY_FRAME.parent_element	= CD_EMGY_PAGE.name
...
ADI_EMGY_ATTITUDE.parent_element	= CD_EMGY_PAGE.name
...
ADI_EMGY_ROLL_MARKER.element_params 	= {"ADI_ROLL",}
ADI_EMGY_ROLL_MARKER.controllers		= {{"rotate_using_parameter" ,0, 1},}
AddElement(ADI_EMGY_ROLL_MARKER)
--****************** RPM Gauge
local RPMDial = MakeDial(-.53, -.835, .25, 0, 0, 270, .01, false, 36, "mainpower", "RPM_NEEDLE", 270, materials["MWHITE"], CD_EMGY_PAGE.name, MakeMaterial(nil, {50, 50, 50, 50})) -- 0, 1.3, 1.3, 200
RPMDial.element_params = {"CD_ENG_TOGGLE"}
RPMDial.controllers    = {{"parameter_compare_with_number", 0, 1}}

for i = 0, 240, 30 do
	local RPMDialLines          = CreateElement "ceSimpleLineObject"
	RPMDialLines.name           = create_guid_string()
	RPMDialLines.material       = materials["MWHITE"]
	RPMDialLines.vertices       = {{0, 0}, {0, .02}}
	RPMDialLines.width          = .005
...
local RPMText    = add_text_with_CD_brightness("RPM%", .175, -.31, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
RPMText.init_rot = {180}

local RPMText90    = add_text_with_CD_brightness("90", -.04, -.31, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
RPMText90.init_rot = {180}

local RPMText60    = add_text_with_CD_brightness("60", .32, 0, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
RPMText60.init_rot = {180}

local RPMNeedle          = create_mfd_tex(MFD_ELEMENTS_D, 463, 824, 674, 855, 1.3, 473)
...
local digitalRPMBox          = CreateElement "ceSimpleLineObject"
digitalRPMBox.name           = create_guid_string()
digitalRPMBox.material       = materials["MWHITE"]
...
local digitalRPM    = white_text_param_with_CD_brightness(-.1075, .15, "RPM_PARAM_U", "%02.0f", RPMDial, mfd_strdefs_digit, "Gripen_Font_WHITE", "LeftCenter")
digitalRPM.init_rot = {180}

local digitalRPM100          = add_text_with_CD_brightness("1", -.1075 + .035, .15, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "LeftCenter")
...
-- Engine temp
local TGTDial          = MakeDial(.15, -.835, .25, 0, 0, 270, .01, false, 36, "mainpower", "TGT_NEEDLE", 270, materials["MWHITE"], CD_EMGY_PAGE.name, MakeMaterial(nil, {50, 50, 50, 50})) -- 0, 1.3, 1.3, 200
TGTDial.element_params = {"CD_ENG_TOGGLE"}
TGTDial.controllers    = {{"parameter_compare_with_number", 0, 1}}

for i = 120, 240, 60 do
	local TGTDialLines          = CreateElement "ceSimpleLineObject"
	TGTDialLines.name           = create_guid_string()
	TGTDialLines.material       = materials["MWHITE"]
	TGTDialLines.vertices       = {{0, 0}, {0, .02}}
	TGTDialLines.width          = .005
...
local TGTText    = add_text_with_CD_brightness("TGT`C", .175, -.31, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
TGTText.init_rot = {180}

local TGTText480    = add_text_with_CD_brightness("480", .31, -.155, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
TGTText480.init_rot = {180}

local TGTText680    = add_text_with_CD_brightness("680", -.07, -.31, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
TGTText680.init_rot = {180}

local TGTText900    = add_text_with_CD_brightness("900", -.31, -.155, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
TGTText900.init_rot = {180}

local TGTNeedle          = create_mfd_tex(MFD_ELEMENTS_D, 463, 824, 674, 855, 1.3, 473)
...
local digitalTGT    = white_text_param_with_CD_brightness(-.0675, .15, "TGT_PARAM", "%02.0f", TGTDial, mfd_strdefs_digit, "Gripen_Font_WHITE", "LeftCenter")
digitalTGT.init_rot = {180}



--white_text_param_with_CD_brightness(-0.5, -0.2, "CUR_MACH", "%0.2f", CD_EMGY_PAGE, mfd_strdefs_digit, "Gripen_Font_WHITE")
local Mach_text_EMGY = add_text_with_CD_brightness("M", -0.72, -0.2, CD_EMGY_PAGE, "Gripen_Font_WHITE" )

local Mach_indicator 				= CreateElement "ceStringPoly"
Mach_indicator.name 				= create_guid_string()
Mach_indicator.parent_element		= CD_EMGY_PAGE.name
...
add_text_with_CD_brightness(".", -0.011, 0, Mach_indicator, "Gripen_Font_WHITE",mfd_strdefs_digit )

local Mach_indicator2 				= CreateElement "ceStringPoly"
Mach_indicator2.name 				= create_guid_string()
Mach_indicator2.parent_element		= CD_EMGY_PAGE.name
...
Mach_indicator2.controllers 		= {{"text_using_parameter",0},{"parameter_in_range" ,0, 0.995, 3}}
...
white_text_param_with_CD_brightness(-0.5, 0.7, "AoA", "%0.0f", CD_EMGY_PAGE, mfd_strdefs_digit, "Gripen_Font_WHITE")
...
ALFA_SYMBOL_EMGY.parent_element	 = CD_EMGY_PAGE.name
...
FuelSquare.parent_element	= CD_EMGY_PAGE.name
AddElement(FuelSquare)

FUEL_TXT = add_text_with_CD_brightness("FUEL", 0.755, 1.275, CD_EMGY_PAGE, "Gripen_Font_WHITE", mfd_strdefs_digit_f )
FUEL_TXT_D = white_text_param_with_CD_brightness(-0.021,-0.15 , "FUEL", "%0.0f", FUEL_TXT, mfd_strdefs_digit_f, "Gripen_Font_WHITE")
add_text_with_CD_brightness("%", 0.115,0, FUEL_TXT_D, "Gripen_Font_WHITE", mfd_strdefs_digit_b )
...
AIRSPEED_EMGY_SCALE.parent_element	= CD_EMGY_PAGE.name
...
AIRSPEED_EMGY_ARROW.parent_element	= CD_EMGY_PAGE.name
AIRSPEED_EMGY_ARROW.element_params  = {"EMGY_IAS"}
AIRSPEED_EMGY_ARROW.controllers	    = {{"move_up_down_using_parameter", 0, 0.0001079*1.1125}}
AddElement(AIRSPEED_EMGY_ARROW)

AIRSPEED_EMGY_READOUT = white_text_param_with_CD_brightness(-0.15,0.0 , "txtCAS", "%0.0f", AIRSPEED_EMGY_ARROW, mfd_strdefs_digit, "Gripen_Font_WHITE") 
...
HEADING_ARROW_EMGY.parent_element	= CD_EMGY_PAGE.name
...
HEADING_TAPE_EMGY.element_params  	= {"EMGY_HEADING"}
HEADING_TAPE_EMGY.controllers	 	= {{"move_left_right_using_parameter",0, -0.00061001} }
AddElement2(HEADING_TAPE_EMGY)

local MAG_text = add_text_with_CD_brightness("MAG", 0.215,0.215, HEADING_ARROW_EMGY, "Gripen_Font_WHITE", mfd_strdefs_digit_XS )
...
ALTITUDE_SCALE_EMGY.element_params  	= {"EMGY_ALTITUDE"}
ALTITUDE_SCALE_EMGY.controllers	 		= {{"move_left_right_using_parameter",0, -0.00001903} }
...
ALTITUDE_SCALE_EMGY2.element_params  	= {"EMGY_ALTITUDE"}
ALTITUDE_SCALE_EMGY2.controllers	 		= {{"move_left_right_using_parameter",0, -0.00001903} }
...
ALTITUDE_SCALE_EMGY3.element_params  	= {"EMGY_ALTITUDE"}
ALTITUDE_SCALE_EMGY3.controllers	 		= {{"move_left_right_using_parameter",0, -0.00001903} }
...
ALTITUDE_SCALE_EMGY4.element_params  	= { "EMGY_ALTITUDE"}
ALTITUDE_SCALE_EMGY4.controllers	 		= {{"move_left_right_using_parameter",0, -0.00001903} }
...
ALTITUDE_EMGY_ARROW.parent_element	= CD_EMGY_PAGE.name
...
VSI_SCALE_EMGY.parent_element	= CD_EMGY_PAGE.name
VSI_SCALE_EMGY.element_params  = {"CD_VSI_TOGGLE"}
VSI_SCALE_EMGY.controllers	    = {{"parameter_compare_with_number",0, 1}}
...
VSI_SCALE_EMGY_ARROW.parent_element	= CD_EMGY_PAGE.name
VSI_SCALE_EMGY_ARROW.element_params  = {"EMGY_VS", "CD_VSI_TOGGLE"}
VSI_SCALE_EMGY_ARROW.controllers	 = {{"move_up_down_using_parameter", 0, 0.000010875},{"parameter_compare_with_number",1, 1}}
...
DISPLAY_TEMP_EMGY.parent_element	= CD_EMGY_PAGE.name
DISPLAY_TEMP_EMGY.element_params  = {"CD_TEMP_TOGGLE"}
DISPLAY_TEMP_EMGY.controllers	    = {{"parameter_compare_with_number",0, 1}}
...
EMGY_BOX.element_params 	= {"CD_EMGY_MODE"}
EMGY_BOX.controllers   		= {{"parameter_compare_with_number",0, 1}}
AddElement(EMGY_BOX)
...
TEMP_BOX.element_params 	= {"CD_TEMP_TOGGLE"}
TEMP_BOX.controllers   		= {{"parameter_compare_with_number",0, 1}} --
AddElement(TEMP_BOX)
...
ENG_BOX.element_params 		= {"CD_ENG_TOGGLE"}
ENG_BOX.controllers   		= {{"parameter_compare_with_number", 0, 1}} --,{"parameter_compare_with_number",1, 1}
AddElement(ENG_BOX)
...
VSI_BOX.element_params 		= {"CD_VSI_TOGGLE"}
VSI_BOX.controllers   		= {{"parameter_compare_with_number",0, 1}} --
AddElement(VSI_BOX)