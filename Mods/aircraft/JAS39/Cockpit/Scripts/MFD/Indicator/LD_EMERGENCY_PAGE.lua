...
EMGY_PAGE.element_params = {"LD_EMGY_MODE", "mainpower"}
...
Light_Black_Background.vertices        = {{-1, 1.4}, {1, 1.4}, {1, -1.4}, {-1, -1.4}}
...
ADI_EMGY_BACKGROUND.init_pos        = {.62}
...
local ADI_EMGY_FRAME          = create_mfd_tex(ADI_FRAME_EMGY, 10, 15, 1260, 1260, .92)
ADI_EMGY_FRAME.name           = create_guid_string()
ADI_EMGY_FRAME.init_pos       = {0, .270}
ADI_EMGY_FRAME.parent_element = EMGY_PAGE.name
AddElement(ADI_EMGY_FRAME)

local ADI_EMGY_ATTITUDE          = create_mfd_tex(ADI_FRAME_EMGY, 1400, 905, 1915, 985, .92)
ADI_EMGY_ATTITUDE.name           = create_guid_string()
ADI_EMGY_ATTITUDE.init_pos       = {0, .23}
ADI_EMGY_ATTITUDE.parent_element = EMGY_PAGE.name
AddElement(ADI_EMGY_ATTITUDE)

local ADI_EMGY_ROLL_MARKER          = create_mfd_tex(ADI_FRAME_EMGY, 1975, 0, 2038, 1143.27 - 110, 1)
ADI_EMGY_ROLL_MARKER.name           = create_guid_string()
ADI_EMGY_ROLL_MARKER.init_pos       = {-.0065}
...
local RPMDial = MakeDial(-.53, -.835, .25, 0, 0, 270, .01, false, 36, "mainpower", "RPM_NEEDLE", 270, materials["MWHITE"], EMGY_PAGE.name, MakeMaterial(nil, {50, 50, 50, 50})) -- 0, 1.3, 1.3, 200
RPMDial.element_params = {"LD_ENG_TOGGLE"}
...
local RPMText    = add_text_with_brightness("RPM%", .175, -.31, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
RPMText.init_rot = {180}

local RPMText90    = add_text_with_brightness("90", -.04, -.31, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
RPMText90.init_rot = {180}

local RPMText60    = add_text_with_brightness("60", .32, 0, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
...
local digitalRPM    = white_text_param_with_brightness(-.1075, .15, "RPM_PARAM_U", "%02.0f", RPMDial, mfd_strdefs_digit, "Gripen_Font_WHITE", "LeftCenter")
digitalRPM.init_rot = {180}

local digitalRPM100          = add_text_with_brightness("1", -.1075 + .035, .15, RPMDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "LeftCenter")
...
-- Engine temp
local TGTDial          = MakeDial(.15, -.835, .25, 0, 0, 270, .01, false, 36, "mainpower", "TGT_NEEDLE", 270, materials["MWHITE"], EMGY_PAGE.name, MakeMaterial(nil, {50, 50, 50, 50})) -- 0, 1.3, 1.3, 200
TGTDial.element_params = {"LD_ENG_TOGGLE"}
...
local TGTText    = add_text_with_brightness("TGT`C", .175, -.31, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
TGTText.init_rot = {180}

local TGTText480    = add_text_with_brightness("480", .31, -.155, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
TGTText480.init_rot = {180}

local TGTText680    = add_text_with_brightness("680", -.07, -.31, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
TGTText680.init_rot = {180}

local TGTText900    = add_text_with_brightness("900", -.31, -.155, TGTDial, "Gripen_Font_WHITE", mfd_strdefs_digit, "CenterCenter")
...
local digitalTGT    = white_text_param_with_brightness(-.0675, .15, "TGT_PARAM", "%02.0f", TGTDial, mfd_strdefs_digit, "Gripen_Font_WHITE", "LeftCenter")
...
Mach_indicator.init_pos       = {-.67, -.2}
Mach_indicator.alignment      = "LeftCenter"
Mach_indicator.stringdefs     = mfd_strdefs_digit
Mach_indicator.formats        = {"%.2f"}
Mach_indicator.element_params = {"machWhole"}
Mach_indicator.controllers    = {{"text_using_parameter", 0}, {"parameter_in_range", 0, .995, 9}}
...
Mach_indicator2.init_pos       = {-.6, -.2}
Mach_indicator2.alignment      = "LeftCenter"
Mach_indicator2.stringdefs     = mfd_strdefs_digit
Mach_indicator2.formats        = {"%.0f"}
Mach_indicator2.element_params = {"machDecimals"}
Mach_indicator2.controllers    = {{"text_using_parameter", 0}, {"parameter_in_range", 0, 1, 99.5}}
AddElement(Mach_indicator2)

add_text_with_brightness(".", -.011, 0, Mach_indicator2, "Gripen_Font_WHITE", mfd_strdefs_digit)
...
VSI_SCALE_EMGY.element_params  = {"LD_VSI_TOGGLE"}
...
VSI_SCALE_EMGY_ARROW.element_params  = {"EMGY_VS", "LD_VSI_TOGGLE"}
...
DISPLAY_TEMP_EMGY.element_params  = {"LD_TEMP_TOGGLE"}
...
EMGY_BOX.element_params 	= {"LD_EMGY_MODE"}
...
TEMP_BOX.element_params 	= {"LD_TEMP_TOGGLE"}
...
ENG_BOX.element_params 		= {"LD_ENG_TOGGLE"}
...
VSI_BOX.element_params 		= {"LD_VSI_TOGGLE"}
